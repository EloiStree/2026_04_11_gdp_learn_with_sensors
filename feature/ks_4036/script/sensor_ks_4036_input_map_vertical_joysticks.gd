class_name SensorKs4036InputMapVerticalJoysticks
extends Node
...
signal on_both_wheels_percent_power_changed(left_wheel_percent_power: float, right_wheel_percent_power: float)
...
var previous_left_input: float = 0.0
var previous_right_input: float = 0.0
...
func _process(delta: float) -> void:
	...
	if left_input != previous_left_input or right_input != previous_right_input:
		previous_left_input = left_input
		previous_right_input = right_input
		on_both_wheels_percent_power_changed.emit(left_input, right_input)
